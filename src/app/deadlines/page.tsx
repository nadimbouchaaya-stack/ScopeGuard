  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--bg-page, #07090F)" }}>
      ...
      {projects.length === 0 ? (
        ...
        <>
          {/* Revenue Summary */}
          <div className="bg-[#111111] border border-[#2A2A2A] rounded-[12px] p-8 mb-8 flex flex-col items-center">
            ...
            {projects.map((project) => {
              ...
              return (
                <Link
                  ...
                  key={project.id}
                  className={`bg-[#111111] border border-[#2A2A2A] rounded-[10px] p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 hover:bg-[#334155] cursor-pointer transition-colors ${days < 0 ? "border-l-[3px] border-l-[#EF4444]" : days <= 7 ? "border-l-[3px] border-l-[#FCD34D]" : "border-l-[3px] border-l-[#34D399]"}`}
                >
